...
def preprocess_image(image, cuda=True):
    ...
    preprocessed_image = crop(preprocessed_image, 224, 224)[:2,...]
...
def test_full_image_network(video_path, model_path, output_path,
    ...
    reader_rgb = cv2.VideoCapture(video_path.replace('flow_videos','videos'))
    ...
    # while reader.isOpened():
        _, flow_image = reader.read()
        _, image = reader_rgb.read()
        ...
        if len(faces):
            ...
            cropped_face = flow_image[y:y+size, x:x+size]
...
if __name__ == '__main__':
    ...
    p.add_argument('--video_path', '-i', type=str,
                   default='demo_dataset/manipulated_sequences/Deepfakes/c23/flow_videos/010_005.mp4')
    p.add_argument('--model_path', '-mi', type=str, default='pretrained/flow/flow_c23_all_ff.tar')
